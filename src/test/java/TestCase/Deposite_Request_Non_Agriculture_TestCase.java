...
public class Deposite_Request_Non_Agriculture_TestCase extends BaseClass {

	// Non_Agriculture deposit type not applicable for general Deposit and
	// Government Agency so that below Code still not applicable but in future if
	...
	void Deposite_Request_Non_Agriculture_Maker() throws IOException {
		try {
			...
			TestPages.Deposite_Request_Non_Agriculture_Maker Depo = new TestPages.Deposite_Request_Non_Agriculture_Maker(
					driver, Wait);
			LoginPages Login = new LoginPages(driver, Wait);
			Login.PortalLogin("rp-Abhishek", "user51", "121@test");
			Depo.Deposite_Request_Non_Agriculture();
