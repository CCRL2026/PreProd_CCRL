...
//100673000019391
	...
	public void CM_RePledge_Request_Maker() throws InterruptedException {
		...
		try {
			if (Records_Not_Found.isDisplayed()) {
				Records_Not_Found.click();
			} else {
				System.out.println("Records_Not_Found pop up is not visible");
			}
		} catch (ElementClickInterceptedException e) {
			System.out.println("Normal click failed, trying JavaScript Records_Not_Found click...");
			js.executeScript("arguments[0].click();", Records_Not_Found);
		} catch (NoSuchElementException e) {
			System.out.println("Records_Not_Found not found: " + e.getMessage());
		} catch (Exception e) {
			System.out.println("Unexpected error for Records_Not_Found: " + e.getMessage());
		}

		try {
			if (String.valueOf(CM_Client_Id).matches("^[0-9]{0,15}$")) {
				Wait.until(ExpectedConditions.elementToBeClickable(client_id_Txt))
						.sendKeys(String.valueOf(CM_Client_Id));
			} else {
		...
		Wait.until(ExpectedConditions.elementToBeClickable(pledge_Req_No_Txt)).sendKeys(CM_pledge_Req_No);
